...
- 模板原则：语义识别先于排版，内容与样式分离；模板来自代码白名单并以可逆补丁应用，
  不允许模型自由生成导言区，XeLaTeX 可用时编译失败即回退原文
...
| R18 | **OCR 转写模块**：PDF/图片 → 逐页忠实转写 LaTeX（模型可选），显示分页进度并支持失败页重试；raw OCR 单独保留，全部页面完成后由用户明确选择 AI/规则整理与排版模板；导入后进入标准可暂停后台任务，实时展示结构草稿，不把 OCR 与重构合并为一次不可审阅操作 | P4, v0.2；v1.0 收敛 | P0 |
...
| ⑥ 导出 | 二选一：单个 ElegantBook TEX；或完整工程 ZIP（主 TEX、原项目图片/子文件、`elegantbook.cls`、许可证、编译说明与汇报）。两者均复用同一安全提交记录 |
| ⑦ OCR 转写 | PDF/图片导入、页码范围、OCR 模型、逐页进度、持续增长的原始 TEX、单页预览/重跑；完成后明确选择 AI 或规则整理 |
...
POST   /api/projects/{id}/exports/{artifact}/save  可靠保存 result/package/report 到固定下载目录
