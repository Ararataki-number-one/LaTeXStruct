...
- 文档版本：1.0（实现同步；后续章节保留早期决策背景）
- 同步日期：2026-08-17
- 状态：v1.1.9 OCR 恢复、资源完整性与可导出性修复
- 交付形态：**本地桌面客户端**（单人使用，后续可打包分享；核心 UI 为内嵌 Web 界面）
- 整理引擎：**AI 大模型**（可配置；默认 DeepSeek V4 Flash/Pro，Qwen 可一枚 Key 覆盖决策/复查/OCR）
- 默认排版：所有正式成品统一使用随应用固定的 **ElegantBook v4.7**；旧项目只有在用户重新整理时
  才迁移，原始 TEX 始终保留，不能把旧安全记录冒充为新模板成品
- 模板原则：语义识别先于排版，内容与样式分离；模板来自代码白名单并以可逆补丁应用，
  不允许模型自由生成导言区；XeLaTeX 可用时编译失败会保留诊断草稿并标记为未验证，用户仍可
  取回当前 TEX/工程包，但不能把它冒充已验证成品
...
| R18 | **OCR 转写模块**：PDF/图片 → 逐页忠实转写 LaTeX（模型可选），显示分页进度并支持安全暂停/继续、单页与全部失败页重试；raw OCR 以 TEX + 图片 + manifest 工程包单独保留，全部页面完成后由用户明确选择 AI/规则整理；导入后进入标准可暂停后台任务，实时展示结构草稿，不把 OCR 与重构合并为一次不可审阅操作 | P4, v0.2；v1.1.9 收敛 | P0 |
...
| D10 | 复查模型 | **可选择，默认该供应商较强模型**（DeepSeek 默认 `deepseek-v4-pro`；省钱可切 `deepseek-v4-flash`） |
...
> 用户拖入一本数学书的 PDF，选择 OCR 模型与页码范围，先取得可逐页检查、持续增长的原始 LaTeX；完成后选择 AI 深度整理或规则整理。项目立即进入后台工作台，逐批显示结构化草稿，统一生成 ElegantBook 成品，并输出安全检查与待人工确认清单。原始 OCR 与结构化成品分开保存。
...
- 复查模型可配置为与决策模型不同（如决策用 V4 Flash、复查用 V4 Pro），三角色独立配置；
...
|---|---|
| 复查开关 | 开启（可关） |
| 复查模型 | 与决策模型同角色配置可不同（如决策 V4 Flash / 复查 V4 Pro，按成本自选） |
| 采样比例 | 未改动高风险段抽样 5%（可配） |
| 上下文 M | 6 段 |
...
flowchart LR
  ...
  P2 --> P3[合并原始 OCR LaTeX<br/>逐页可检查/重试]
  P3 --> S[AI 深度整理或规则整理]
  S --> E[固定 ElegantBook 成品<br/>编译与安全检查]
```
...
- OCR 阶段只合并可追溯的原始 LaTeX，不让视觉模型自由生成最终模板；进入结构化流水线后，
  再由确定性模板补丁统一成 ElegantBook（见第 13 节）；
...
- 正式成品固定：`\documentclass[lang=cn,scheme=chinese,11pt]{elegantbook}`（英文输入用 `lang=en`）；
  `elegantbook.cls` v4.7 与许可证随应用和完整工程包提供，并在使用前校验固定 SHA-256；
- 未提供真实封面图时使用自包含的简洁标题页，禁止落回 `example-image` 占位图；目录使用
  `frontmatter` 页码，正文经 `mainmatter` 从第 1 页重新编号；
- 模型只识别章节、目录和定理语义，不能自由生成导言区。模板转换由白名单补丁完成；已有
  ElegantBook 声明与环境沿用，缺失的受支持环境才按固定定义补齐。

### 13.2 环境适配（替代"默认补 amsthm"）

| 情况 | 行为 |
|---|---|
| 文档类为 elegantbook | 沿用其内置定理环境体系；定理类统一使用无编号星号色块，OCR/原书显式编号作为可见标题参数保留，避免模板自动编号与原编号重复；缺失的猜想、注记、例题、问题和练习环境用固定 `\elegantnewtheorem` 定义补齐 |
...
| 探测方式 | 扫描源文件导言区 `\newtheorem/\newenvironment/\elegantnewtheorem`，并使用随应用固定的 ElegantBook 环境白名单；不依赖用户机器上版本不确定的类文件 |

### 13.3 双语标题与目录（ElegantBook 场景）

- 文章型输入整体按语义层级确定性上移：`section→chapter`、`subsection→section`、
  `subsubsection→subsection`，避免书籍类出现 `0.1`；书籍型输入保留真实 `chapter`；
- 只保留或整理源文件/OCR 大纲已经确认的 `\tableofcontents` 与目录命令，不根据“1 Title 3”一类
  数字文本猜测目录，也不擅自删除手工内容；
- ElegantBook 常见"英文标题 + 中文翻译 tcolorbox"模式仍按 R5 的严格条件合并；
...
| ⑥ 导出 | 二选一：单个 TEX；或完整工程 ZIP（主 TEX、原项目图片/子文件、`elegantbook.cls`、许可证、编译说明与汇报）。通过检查时导出已验证成品；失败或未完成审阅时导出带 `UNVERIFIED` 警告的当前快照 |
| ⑦ OCR 转写 | PDF/图片导入、页码范围、OCR 模型、逐页进度、安全暂停/继续、持续增长的原始 TEX、单页/全部失败页重跑；完成后可保存 TEX + 图片工程包并明确选择 AI 或规则整理 |
| ⑧ 设置 | 三角色模型配置（决策/复查/OCR：供应商/模型/Key）、预算上限、复查开关与高级参数；正式模板不再要求新手选择 |
...
GET    /api/projects/{id}/export             导出已验证的 ElegantBook 主 TEX
GET    /api/projects/{id}/export-package     导出完整 ElegantBook 工程 ZIP
GET    /api/projects/{id}/export-current     导出当前 TEX（失败时明确标记 UNVERIFIED）
GET    /api/projects/{id}/export-current-report 导出当前汇报（未验证时带警告文件名）
GET    /api/projects/{id}/export-current-package 导出当前工程 ZIP（含资源、汇报与警告）
POST   /api/projects/{id}/exports/{artifact}/save  可靠保存 verified/current TEX、ZIP 或汇报到固定下载目录
POST   /api/projects/{id}/latexcheck         本地编译自检（可选）
POST   /api/ocr/inspect                      OCR：上传一次并探测 PDF 总页数/图片任务
POST   /api/ocr/jobs/{id}/start              幂等启动页码范围任务
GET    /api/ocr/jobs/{id}                    OCR 进度、页状态与成本
GET    /api/ocr/jobs/{id}/preview            按 revision 获取持续增长的原始 TEX
POST   /api/ocr/jobs/{id}/pause              当前页结束后安全暂停
POST   /api/ocr/jobs/{id}/resume             从下一未完成页继续
POST   /api/ocr/jobs/{id}/retry-failed       顺序重试全部失败页
GET    /api/ocr/jobs/{id}/package            下载 raw OCR TEX + 图片 + manifest ZIP
POST   /api/ocr/jobs/{id}/import             选择 AI/规则并进入标准后台结构化工作台
...
| `roles.decide.model` | deepseek-v4-flash | 决策模型 |
| `roles.review.model` | DeepSeek：`deepseek-v4-pro` | 复查模型可选，省钱可切回 `deepseek-v4-flash` |
...
| 指标 | 门槛 |
|---|---|
| 内容不变校验通过率（标为“已验证”的导出文件） | **100%**；未通过的当前快照必须使用 `UNVERIFIED` 文件名、响应头和包内警告，绝不冒充成品 |
| `\begin/\end` 配平违规 | 0 |
| 已正确环境被二次包裹 | 0（C3） |
...
**v0.7/v0.8 产品化阶段 · 第 2 轮（Benchmark 扩充，v0.6.2）：**
1. **真实书稿切片金标**：从两本书切 10 组开发集 + 2 组 hold-out（godsil 1.8/2/3/8/10、
   extremal 1/2/3/6/8；hold-out：godsil 4、extremal 12，声明不用于开发调试），
   标签 213 个（开发 179 + hold-out 34），**全部真实标题**（Orbit-stabilizer/Deza/
   Füredi/Tutte/Definition 12.1 等，抽查确认）；标签改**行号锚定**（needle 仅作可读性），
   解决裸标题行（"Proof. If"）无法用文本 needle 区分的问题；
2. **报告升级**：汇总行含总 P/R/F1 + **正文零改动率 / 引用保持率 / 编译成功率**；
   `--compile` 语义收紧为"仅声明可编译的金标参与对比"（编译成功率 100%）；
3. **基线对比环境平衡**（本轮发现并修复的通用健壮性缺陷）：整书切片等部分输入原文自身
   环境不平衡时，门禁从"绝对平衡"改为"**不新增不平衡**"（前后不平衡集合一致即通过），
   使部分文档也可安全处理；
4. 当前成绩：14 金标集 / 200 标签，**TP 200、FP 0、FN 0**，全 100%；109/109 测试全绿。

**v0.7/v0.8 产品化阶段 · 第 3 轮（React+Vite+Monaco 基础设施，v0.7.0）：**
1. **前端重构**：React 18 + Vite + Monaco（本地打包 worker，无 CDN，桌面离线可用），
   构建产物 `server/static-react`（React 为主界面，旧原生 UI 移至 `/legacy`）；
   CI 在 PyInstaller 前自动 `npm install && npm run build`，打进安装器；
2. **后端能力全暴露**：`GET /api/rulesets`、文件夹导入 `POST /api/projects/folder`
   （文件映射 → 临时目录 → 多文件处理 → 项目+依赖图）、`GET /api/projects/{pid}/graph`、
   `GET /api/projects/{pid}/export-folder`（结构化文件夹 zip）；项目元数据新增 pack 字段；
3. **界面能力**：项目页（粘贴/文件/文件夹选择器 + 模式/规则包/模板选择 + 依赖图摘要）、
   工作台（Monaco 双编辑器 + 决策清单 + 单项拒绝 + zip 导出）、设置页（三角色模型）；
4. 修复挂载顺序（"/" 吞 /legacy）、zip 端点 HTTP 方法、process_project pack 透传等 3 处问题；
   110/110 测试全绿。

**v0.7/v0.8 产品化阶段 · 第 4 轮（Monaco 三栏审阅工作台 + OCR 逐页验证，v0.7.1）：**
1. **三栏审阅工作台**（Workspace.jsx）：左栏决策树（按章节分组、状态着色、置信度角标）→
   点击后 Monaco **DiffEditor** 双向对照（行号居中定位）→ 右栏检视器（置信度/原因/来源 +
   **拒绝此项 / 拒绝同类其余 / 全部重置**）；后端新增 `POST /decisions/reject-batch`（同类
   批量拒绝）与 `POST /decisions/reset`（清空排除清单重跑），拒绝后整链路重新校验；
2. **OCR 逐页验证**（Ocr.jsx）：任务进度按页细粒度展示（pending/running/done/error +
   低置信度标注），页面 PNG 预览与 LaTeX 转写并排校对，**单页重试**端点
   `POST /api/ocr/jobs/{jid}/pages/{n}/retry` + `GET .../pages/{n}`（PNG）与
   `.../pages/{n}/tex`（转写）；无 Key 时任务优雅完成、逐页报错（errors 列表）；
3. **工程收口**：服务器模块闭包跨作用域引用修复（transcribe_page 局部导入 + 回调挂载到
   job 后状态接口过滤内部键）、`/legacy` 无斜杠 404 重定向修复、dbgo.txt 调试残留清理；
   新增 per-page 端点服务测试，113/113 测试全绿、ruff 全绿。

**v0.7/v0.8 产品化阶段 · 第 5 轮（密钥安全加固，v0.8.0）：**
1. **系统凭据管理器（keyring）**（`latexstruct/keystore.py`）：Windows Credential Manager
   （advapi32 CredWriteW/CredReadW/CredDeleteW 经 ctypes 零依赖封装，Generic 类型凭据、
   目标名 `LaTeXStruct/<字段>`），后端抽象可注入（测试用 FakeBackend，真实凭据不进测试）；
2. **配置层改造**（config.py）：AppConfig 新增 `keyring` 开关；开启后 save_config 把
   三个 API Key 写入系统凭据，config.json 只存 `__keyring__` 占位符（明文绝不落盘）；
   关闭时自动清理系统凭据防残留；load_config 解析占位符回真实密钥并记录来源标记，
   masked() 区分「已配置(系统凭据)」/「已配置」；环境变量覆盖时清除来源标记；
3. **设置页**：一键开关「使用系统凭据管理器保存密钥」+ 占位符文案显示来源；
   服务端 ConfigRequest/put_cfg 支持 keyring（保存后重载解析）；
4. **测试防污染修复**：原 test_config_masked 会覆写用户真实 config.json——改为注入
   FakeBackend + 临时 CONFIG_PATH 的 hermetic 测试（顺带验证 keyring 全链路）；新增
   test_keystore 套件 6 例（往返/占位符不落盘/关闭清理/清空解析/平台可用性）；
   119/119 测试全绿、ruff 全绿；CI 测试清单扩至 18 套件。

**v0.7/v0.8 产品化阶段 · 第 6 轮（收尾回归 + 发布，v0.8.0）：**
1. 全量回归：ruff（F+E741+W605）全绿、18 套件 119/119 测试全绿；真实服务冒烟
   （React 主界面 / /legacy 重定向 / health / config keyring 字段 / OCR 逐页链路）；
2. 发布：v0.7.1（Round 4）与 v0.8.0（Round 5-6）两条 tag CI 全绿，安装器构建 +
   冒烟测试 + Release 发布成功；README 更新至当前形态（keyring/三栏工作台/18 套件）；
3. 目标达成：用户评审提出的「从功能堆叠转向可信、好用、可扩展」在六轮内落地——
   见下方产品化总报告。

### v0.7/v0.8 产品化阶段总报告（2025-08，六轮收官）

| 维度 | 措施（对应轮次） | 证据 |
|---|---|---|
| **可信**（只改结构不改内容） | 可逆补丁 + 内容不变逐字符校验 + 多层不变量（数学 token 多重集、引用集合、环境平衡"不新增不平衡"）+ Compile CI 编译对比门禁（R1/R2） | 金标 200 标签 TP 200 / FP 0 / FN 0；正文零改动率/引用保持率/编译成功率 100%；CI 每 tag 强制 18 套件 119 测试 + TinyTeX 编译对比 |
| **可信**（密钥安全） | API Key 入 Windows 凭据管理器，config.json 不再落明文（R5） | keyring 全链路 hermetic 测试 6 例；设置页来源标识 |
| **好用**（审阅体验） | Monaco 三栏审阅工作台：决策树分组 / 双向 diff 行号定位 / 检视器（拒绝此项、拒绝同类、全部重置）；React 重写 + 离线 Monaco（R3/R4） | 三栏联动冒烟通过；旧 UI 保留在 /legacy |
| **好用**（OCR 验证） | 逐页状态 + 页面 PNG 预览 + 转写并排校对 + 低置信度标注 + 单页重试（R4） | per-page 端点测试 + 无 Key 优雅降级测试 |
| **可扩展** | 规则包（Rule Pack JSON）+ 多文件项目（依赖图）+ 三角色模型路由 + 可注入后端（keyring/存储/AI 客户端）（R2/R3/R5） | 17→18 测试套件全程守护；Benchmark --holdout 独立评估 |
| **工程债** | 版本单一事实来源 + lint 升级 + 安装器冒烟测试 + TinyTeX 自愈下载 + jobs.json 残留清理（R1） | CI 从"能跑"到"每次 tag 完整交付安装包"，共 15 个 Releases（v0.2.0→v0.8.0） |

**遗留与下一步**（v0.9/v1.0 候选，按价值排序）：
1. **大规模真实书 Benchmark**：金标从 14 组扩展到多本整书规模（数百标签、分章节报告），
   建立回归护栏防止提示词/规则改动引发静默退化（当前 hold-out 仅 2 组 34 标签）；
2. **多级验证可视化**：把不变量校验结果（token 多重集/引用集合/环境平衡）展示到审阅面板，
   让"机器验证正文没变"对用户可见可查；
3. **代码签名证书**：购买 EV/OV 证书消除 SmartScreen 警告（CI 签名步骤已就绪，仅缺证书）；
4. 低置信度门禁（置信度阈值自动转歧义清单）、安装器增量更新（减少下载体积）。

**第 8 轮（审阅式 UI + 总报告，v0.6.0）：**
1. **决策清单**（PipelineResult.decision_items）：候选元信息（类型/环境/行号/章节路径/
   标题/置信度/来源/原因）+ 状态（applied/rejected/ambiguous/none）；
2. **单项拒绝重跑**：`run_pipeline(exclude=...)` 排除指定决策后重新应用全部补丁并
   重新做内容不变/多层不变量/环境配平校验；服务端 `POST /decisions/{cid}/reject`
   （拒绝清单持久化在项目 meta）+ `GET /decisions`；
3. **审阅面板 UI**：左侧决策清单（按章节显示、状态着色）→ 点击查看详情（置信度/原因/
   来源）→ **拒绝此修改（恢复原文）**；过滤器（全部/只看 Proof 边界/只看歧义/高置信）；
   diff 行带行号定位，点击决策自动滚动到对应位置；
4. 107/107 测试全绿（新增决策清单+拒绝接口服务测试）。

### 评审重构记录（2025，按用户代码评审 P0/P1/P2 路线）

**v0.7/v0.8 产品化阶段 · 第 1 轮（工程债 + Compile CI，v0.6.1）：**
1. **pyproject 版本核实**：当前仓库已是 `dynamic = ["version"]` + `_version.py` 属性引用，
   不存在 0.1.0 残留（用户看到的为旧快照）；test_version 持续锁定；
2. **Lint 门禁升级**：ruff 从仅 F 规则扩展到 F + E741（歧义变量名）+ W605（转义序列），
   E402/E501 按需忽略，tools 目录豁免 E741；修复全部 9 处问题；
3. **Compile CI 落地**：CI 安装 TinyTeX + 所需包，Benchmark 以 `--compile` 运行——
   金标集按 `"compile": true` 声明参与前后编译对比（当前 basic-book 2 页 /
   cn-fragment 1 页 / negatives 1 页全部编译成功），报告编译列不再是 `—`；
4. 107/107 测试全绿。

### 评审重构总报告（2025）

从"单文件 tex 整理工具"到"大型数学 LaTeX 项目安全重构引擎"的完整重构，8 轮全部落地
并逐轮发布（v0.5.0 → v0.6.0）：

| 轮 | 评审项 | 交付 |
|---|---|---|
| 1 | P0 发布链路/版本/门面 | CI 安装器冒烟测试（抓到 2 个真实缺陷）、版本 SSOT、LICENSE/CHANGELOG/设计文档 |
| 2 | P2 多层校验 | 公式 token/`\label`/`\ref`/`\cite`/图片路径不变量 + xelatex 前后编译对比 |
| 3 | P1 parser 升级 | 平衡括号解析（嵌套标题/数学内花括号/可选参数），真实书稿零回归 |
| 4 | P1 多文件项目 | main 发现/`\input` 依赖图/标记展开→流水线→拆分导出 |
| 5 | P1 Rule Pack | 四内置规则包 + 用户自定义 JSON + 全链路 pack 贯通 |
| 6 | P1 Benchmark | 四组金标集 P/R/F1 + 内容指标 + CI 门禁（当前全 100%、零误报） |
| 7 | P1 OCR 解耦 | Stage A 纯忠实转写（禁结构判断）/ Stage B 复用流水线 |
| 8 | P2 审阅 UI | 决策清单 + 单项拒绝重跑 + 过滤器 + diff 定位 |

**当前状态**：107 测试全绿、17 个套件、CI 门禁（ruff/测试/benchmark/安装器冒烟）、
9 个 Release 自动发布；核心定位（用户评审确认）：**A safe AI-assisted refactoring
engine for large mathematical LaTeX projects**——AI 只决策、补丁可逆、机器验证内容不变。
...
2. 复查模型：**可选择**；DeepSeek 默认 `deepseek-v4-pro`，可切 `deepseek-v4-flash`（D10）。
...
## 21. v1.0 功能冻结与打磨工程（2025-08 起）

> 用户决策：**目标做到 v1.0.0 后暂停扩功能**。从现在起冻结功能范围，把 LaTeXStruct 从
> 「功能很多、能跑」打磨到「**稳定、可信、顺手、普通用户拿来就能用**」。
> v1.0 定义 = **一个可以安全地把真实数学 LaTeX 项目交给它进行结构化整理的软件**。

### 21.1 功能冻结政策

自本清单生效起，除以下三类外**不增加功能**：

1. 修 Bug；
2. 补齐已经存在但没做好/没暴露出来的能力；
3. 明显影响核心工作流的问题。

明确**不做**（冻结名单）：Agent、知识图谱、Git 高级能力、插件市场、VS Code 扩展、
新复杂重构、更多花哨 AI 功能；模型接入收敛为 **1～2 个文本 Provider + 1 个视觉
Provider**，把配置做好而不是增加模型名单。

### 21.2 六个打磨方向（v1.0 路线）

| # | 方向 | 要点 |
|---|---|---|
| 1 | 功能冻结 | 见上；冻结名单写入文档并遵守 |
| 2 | 核心链路接近不出错 | parser 边界（嵌套环境/自定义宏/注释/verbatim/minted/数学环境/\input/\include/\ref/\label/中文/奇怪空白/OCR 脏数据/不完整 LaTeX/自定义定理环境）；**宁可不改，也不能错改**——低置信度必须用户确认 |
| 3 | 审阅体验 | 上/下一条、接受/拒绝、全部接受、相似项全部接受、跳转源码、原文/修改后查看、撤销、搜索、按置信度/类型过滤、快捷键（A/R/↑↓/Ctrl+Z） |
| 4 | OCR 做稳不做大 | PDF/PNG/JPG 多页；逐页进度（✓ 1–16 / ⟳ 17 / ○ 18–283）；单页失败只重试该页；输出分离：original.pdf + **ocr_raw.tex**（视觉原始转录）+ **structured.tex**（结构处理后），视觉模型只做忠实转录，不承担理解/重构/美化/改写 |
| 5 | UX 清理 | 新手首启流程：打开 → 拖入 → 分析 → 「发现 N 个结构问题」→ 审阅 → 应用 → 验证通过 ✓ → 打开文件夹；Rule Pack/模式等一律进高级设置，默认只看到 项目/分析/审阅/应用 |
| 6 | 错误信息 | 人类可读错误 + 可能原因列表 + 操作按钮（重新尝试/打开设置）；编译失败明确提示「已自动恢复修改前版本」并给 [查看问题][重新分析] |

### 21.3 安全中心（结果页核心）

结果页直接展示逐项安全检查：正文内容 / 数学公式 / Label / Reference / 图片路径 /
文件数量 / XeLaTeX 编译 / 新增 Warning，每项 ✓/✗，汇总「安全状态：✓ 可以保存」——
直接强化核心产品价值：**我修改的是结构，而不是你的内容**。
...
前端分析期间 UI 不得卡死；关注 CPU/内存/AI 并发/文件读取/parser/diff/大文本渲染。

### 21.5 v1.0 验收清单

项目导入 ✓ · OCR ✓ · 结构识别高准确率 ✓ · AI 模型配置简单 ✓ · Rule 模式 ✓ ·
多文件 ✓ · Diff 清晰 ✓ · Accept/Reject ✓ · Undo ✓ · 内容一致性检查 ✓ ·
编译检查 ✓ · 错误恢复 ✓ · Windows 安装 ✓ · 自动更新 ✓ · 大项目不崩 ✓ ·
Benchmark ✓ · 文档 ✓ · 新用户上手简单 ✓。

### 21.6 现状差距审计（第 1 轮基线）

| 要求 | 现状 | 差距 |
|---|---|---|
| 上/下一条、快捷键、搜索、类型/置信度过滤、原文/修改后切换、撤销 | 仅 全部/Proof/歧义/高置信 4 个过滤 + 点选 + 拒绝/拒绝同类/全部重置 | **本轮补齐** |
| 安全中心 | 不变量校验在 verification.json，界面只显示汇报文本 | 第 2 轮 |
| parser 边界用例 | 单测 119 例，边界用例未成体系（minted/自定义宏/不完整文档等） | 第 3 轮 |
| OCR 输出分离 | 仅合并结果 tex，无 ocr_raw/structured 分离 | 第 4 轮 |
| 性能实测 | 真实书稿 35k/31k 行跑通过；50k+/100k+ 未实测、无性能门槛 | 第 5 轮 |
| 新手流程/高级设置收纳 | 项目页暴露 mode/pack/template 全量选项 | 第 6 轮 |
| 错误信息 | 多数为原始异常文本 | 第 6 轮 |

### 21.7 发布计划

v0.8.1（审阅工作台打磨）→ v0.8.2（安全中心）→ v0.9.0（健壮性+OCR 稳定）
→ v0.9.x（性能/UX/错误信息）→ **v1.0.0**（验收清单全过 + 总报告）。

### 21.8 v1.0.0 收官（2026-08-14）

v1.0.0 按功能冻结政策收官：核心解析与补丁采取保守边界，安全检查统一阻止危险导出；
审阅工作台、OCR 分页恢复、多文件资源保持、可行动错误提示及发布门禁完成系统性打磨。
发布以全量单元测试、金标基准、真实 XeLaTeX 前后编译、前端生产构建和 Windows 安装器
安装/健康检查/卸载为最终准入条件。v1.0.0 之后默认只维护缺陷、安全性与兼容性，
不恢复 Agent、知识图谱、插件市场、VS Code 扩展或复杂新重构等冻结能力。
