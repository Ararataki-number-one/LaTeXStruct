...
```powershell
# 1) 在候选分支完成代码、门禁和发布说明；修改版本后同步元数据
$version = "X.Y.Z"
python packaging/sync_version.py --version $version
# 2) 只按精确路径暂存候选实现；禁止 git add . / git add -A / git add -u
git add -- <本次修改和新增的精确文件路径...>
git status --short
...
- v2.0.0 把“忠实 OCR 与基线恢复”和“OCR 后 AI 自动整理”明确分离。OCR 运行使用不可变快照、
  ...
  TEX 不会被后续语法恢复、结构分析或模板处理覆盖；
- OCR 页面只保留文件、页范围和“快速 / 推荐 / 高质量”三个质量入口，并显示真实页面状态、近期
  速度与动态预计剩余时间。OCR 完成后分别呈现不可变原稿、语法恢复基线和真实编译证据，AI 自动
  整理作为后续独立操作；
...
- 真实页面通路已完成一次合成双图片项目的 Codex CLI OCR 冒烟：2/2 页为 `SUCCESS`、并发数为 2、
  ...
  通路冒烟，不是识别质量或整书性能验收，不可外推整书吞吐；
- Codex CLI 调用现在以运行时实际 feature registry 为权威，只传递当前版本支持的功能开关；启动、
  登录和配置类永久失败会立即终止，不再进行无效的瞬时重试。页面预览在模型调用前完成原子写入
  和持久化，任务耗时覆盖合并、冻结、基线恢复与双遍编译等完整流程，多图片输入则逐张绑定顺序、
  文件名、字节数、SHA-256、派生标识和源集合哈希；
- 界面中的 100% 仅表示本次计划阶段已执行到终点，不等于 `VERIFIED`。独立复核、第二遍 AI 复查、
  编译、视觉闭环或最终门禁任一缺失/失败时，结果仍为 `COMPLETED_WITH_ISSUES`、`UNVERIFIED`
  或失败状态，不会因进度值自动提升；
- v2.0.0 只有在 `analysis-37` 真实完整验收通过后才能发布。该 profile 固定使用
  `SOME RECENT RESULTS IN RAMSEY THEORY.pdf` 的 37 页原文（SHA-256
...
- OCR 导入会从冻结、哈希绑定的源 PDF 字节复算并校验真实总页数，任务快照和 manifest 不再把
  ...
  实际页数和选择范围时才会自动迁移，并留下源 SHA-256 与权威 snapshot ID；
- 工作台新增“输入证据检查”阶段，终态审计材料保存使用独立状态；预检失败会显示真实中断位置，
  不再出现核心阶段均未开始却显示 99% 的误导进度；

- AI 分析与审阅的核心流水线已升级为：宿主全文 formal 清点 →（启用第二遍复查时）独立全文复核 → 哈希绑定的可逆补丁
  → 真实 LaTeX 编译 → 确定性逐页渲染检查 → 视觉模型逐页核对 → 最多两轮定点修复与重新编译
  → 最终 formal 清单和 TEX/PDF 哈希门禁。模型不能自行增删正文或发明文件角色；缺页、异常尺寸、
  漏套、错套、多套、重复、未回答清单项或来源证据缺失都会阻止盖章，并回滚到修复前结果；
- 视觉来源保留可复验血缘：PDF 原始 bytes 直接进入比对；图片会被包装成单页视觉 PDF，同时记录
  原图和派生 PDF 的字节数、SHA-256 与固定派生标识，且明确声明该包装文件不是 LaTeX 编译结果；
- 模板重排改变页数时，宿主会按内容锚点冻结单调页映射并覆盖源/候选全部页面；超过 80 页的整书
  也不会被截断。没有可用文本层时只能降级为待视觉确认，不能自动通过；
- 17 页 Sharp Bounds 固定原始 PDF + 原始 OCR TEX 的独立回归达到 formal/proof 19/19、目录 9/9、
  公式号 6 个、参考文献 27 条并独立编译为 17 页。该结果只说明这一份固定、已审阅样本通过，
  不代表任意 OCR 文字、数学语义或出版质量都能达到 95%；

- AI 审计提交包现拥有独立的打包完整性状态和打包后内容守恒门：TeX、类文件及 TeX 差异仅按
  宿主已知路径或明确凭据脱敏，数学命令不会再被路径规则误删；PARTIAL_COMPILED、真实大纲、
  页范围、阶段执行状态、模板依赖、编译输入清单和完整 provenance 均以机器证据写入 manifest；
- “模型与 API 设置”已按结构判断、AI 复查、图片/PDF OCR 三个职责重新组织，可分别选择模型、
  API Key 与 Base URL，并支持连接测试、Codex CLI、本机凭据加密、高级覆盖和状态反馈；

- 核心流水线同时保留解析、扫描、保守 AI 决策、独立复查、可逆补丁和统一安全检查；
  定理/证明边界、自定义环境、label/ref、图片路径、花括号、项目文件集合、全文复核与编译后逐页
  视觉证据共同决定能否标为已验证成品；
- 导出来源记录显式区分 producer（真正生成结果的版本/构建/提交）与 exporter（本次导出的
  版本/构建/提交）；旧工程没有生成身份证据时保持 `unknown`，不会被新版导出器重新署名。
  原始 TEX 同时记录字节哈希、LF 规范化文本哈希、包内角色与路径，CI 产物嵌入确切构建身份；
- 每个工程 ZIP 带有机器可读的终态运行清单、结构化报告、问题清单、验证与决策记录，以及覆盖
  其余包内成员的 `SHA256SUMS`；输入、原始 TEX、未盖章结果和最终主文件之间的哈希血缘可在
  应用外独立复算，SUCCESS 与 UNVERIFIED/FAILED 等状态不会混淆；Unicode 文件名的校验清单
  固定使用 UTF-8，并拒绝会破坏清单行边界的文件名；
- 编译预览明确分为 `COMPILED`、`PARTIAL_COMPILED`、`SOURCE_PREVIEW`：只有真实、完整的
  LaTeX 编译 PDF 才标为已编译，失败后留下的可读 PDF 与单纯源码预览分别标示，界面不再把
  “能显示源码”解释成“编译通过”。编译 PDF 使用内容哈希隔离保存，并同时绑定 PDF 哈希和
  实际送编主 TEX 的 LF 规范化哈希；全部子 TEX、图片、样式资源另有逐文件字节哈希与总清单哈希。
  导出时会重建并复算完整输入集，任一子文件或资源不一致都会闭锁；失败重试不能覆盖既有成功预览，
  原工程残留的 PDF/日志不能冒充本次结果，旧工程则安全降级到源码预览；
- OCR 严格语义 IR 在目录恢复后、模板转换前清点公式编号、References 和前置信息；只有 PDF
  几何证据与 LaTeX 清单逐项一致时才规范化 AMS `\tag`，并保留源 PDF 的左/右编号位置；只有
  参考文献边界唯一且编号连续时才生成 `thebibliography`，随后逐条检查全部活动 `\bibitem` 的
  唯一编号和多行正文哈希。独立评分器同样保留公式号与文献号，错号不能再被“正文相同”掩盖。
  Sharp Bounds 固定样本回归覆盖 6 个公式号与 27 条参考文献的顺序和正文哈希守恒；任何证据
  缺失、错号、重复或边界歧义都会保留原文并闭锁；
- 工程包对用户文件与来源、报告、运行清单、预览等保留工件执行统一命名空间检查；大小写别名、
  Windows 尾随点/空格、设备名、数据流写法及文件/目录层级冲突都会明确阻止打包，避免静默覆盖；
...
- 审阅工作台支持逐项接受/拒绝、上一项/下一项、筛选、低置信度提示、撤销和源码定位；
  用户调整审阅项时复用已有决策，不重复调用初次结构判断；新任务在用户启用第二遍复查时执行
  独立全文复核，关闭时不会产生复查调用或把跳过显示成通过；OCR 逐页视觉复核会按冻结的运行
  快照和宿主页映射单独执行；
...
- 手动候选 workflow 执行完整测试、构建、Windows 安装/卸载与升级冒烟；tag workflow 重跑
  测试门、核验严格 `analysis-37` 完整验收投影并复算先前候选字节。最终安装器完成签名后还会
  再次静默安装，并逐字节核对其中的 `LaTeXStruct.exe` 与被打包候选 EXE 的 SHA-256；任一门禁
  失败都不会发布，tag 阶段不重建资产。
