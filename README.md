...
- OCR 项目还会校验所选 PDF 书签节点、目录命令与真实图片文件；本机存在 XeLaTeX 时，
  整理结果必须实际编译成功才能导出；
- 正式成品统一使用 ElegantBook v4.7：文章型层级会确定性上移为书籍层级，目录只依据原命令
  或 PDF 大纲生成；定理使用无编号色块承载原书编号，避免 `0.1` 和双编号。模板仍是可逆补丁，
  不允许模型自由生成导言区；没有用户封面图时使用简洁的无占位图标题页，目录与正文页码
  分开编号；旧项目需重新分析后才能按新模板导出；
...
- 成品可选择导出单个 ElegantBook TEX，或完整工程 ZIP；ZIP 同时包含主 TEX、原始图片/子文件、
  ...
  “下载/LaTeXStruct”目录且同名不覆盖；旧模板、hash 不一致或安全检查未通过时一律阻止；
...
5. **导出**：可选单个 ElegantBook TEX，或包含类文件、图片/子文件、许可证与汇报的完整工程 ZIP；
